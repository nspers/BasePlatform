export default class AMapTool {
    ...
    static async loadAndInit(param) {
        await new Promise((resolve, reject) => {
            if (window.AMap) {
                ...
                script.src = '//webapi.amap.com/maps?v=1.4.15&key=a3a9810bca5fbdb3fee6cd7c77fcd927&callback=AMapCallBack&plugin=AMap.MarkerClusterer';
    ...
    constructor({
        ...
        ctx.moveTo(0, 32 * 4);
        ctx.lineTo(canvas.width, 32 * 4);

        ctx.moveTo(32 * 4, 0);
        ctx.lineTo(32 * 4, canvas.height);
        ctx.stroke();
        ...
        for (let i = 1; i < 8; i++) {
            if (i !== 4) {
                ctx.moveTo(0, 32 * i);
                ctx.lineTo(canvas.width, 32 * i);

                ctx.moveTo(32 * i, 0);
                ctx.lineTo(32 * i, canvas.height);
            }
        }
        ctx.stroke();
        ...
        this.amap.on('zoomstart', function(e) {
            if (self.cluster) {
                setTimeout(function() {
                    if (self.cluster.getMap()) {
                        self.cluster.setMap(null);
                    }
                }, 0);
                setTimeout(function() {
                    if (!self.cluster.getMap()) {
                        self.cluster.setMap(self.amap);
                    }
                }, 200);
            }
        });
        this.amap.on('zoomend', function(e) {
            if (self.cluster) {
                setTimeout(function() {
                    if (!self.cluster.getMap()) {
                        self.cluster.setMap(self.amap);
     ...
     */
    removeLimit() {
        this.amap.setZooms([3, 20]);
        this.amap.setLimitBounds(null);
    };
    /**
     * 还原地图到初始状态
     */
    ...
    setAllMarkerCluster() {
        if (!AMap.MarkerClusterer) {
            console.log('MarkerClusterer未加载');
            return null;
        }
